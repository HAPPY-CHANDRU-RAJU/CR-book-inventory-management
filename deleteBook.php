<?php
	...
	$sql7 = "SELECT * FROM `book` WHERE `BookId`='$sid'";
	$res7 = $conn->prepare($sql7);
	$res7->execute();
	$rows = $res7->fetch();

	if($rows['BookStatus'] == "INACTIVE"){
		$sql7 = "UPDATE `book` SET `BookStatus`='ACTIVE' WHERE  `BookId`='$sid'";
	}else{
		$sql7 = "UPDATE `book` SET `BookStatus`='INACTIVE' WHERE  `BookId`='$sid'";
	}
	$res7 = $conn->prepare($sql7);
	if(!$res7->execute()){
		$error = TRUE;
	...
	if(!$error){
		...
		header("Location: dashboard.php?bookShow=TRUE");
		exit;
	} else {
		$stat = "danger";
...
<head>
	...
	<title>CR Book Inventory Management - Delete Book</title>
</head>
<body>
<?php
    ...
    margin: 50px;">
    	...
    	<div class="col-md-12">
        	<div class="form-group">
            	<h2 class="text-default" style="color: #ffffff"> <b>Delete Your Book </b></h2>
            </div>
